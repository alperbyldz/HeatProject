...
#COLD_HEAT_TRANSFER_COEFF = ?  # we are going to calculate W/(m²·K)
#hot => mix
HOT_TEMP_IN = 80  # °C
HOT_TEMP_OUT = 50  # °C
...
#HOT_HEAT_TRANSFER_COEFF = ?  # we are going to calculate W/(m²·K)
                ...
                if dirt_factor >= 0:
                    if type(dirt_factor) == float:
                        ...
                        if total_P_drop < 172368.925:
                            results.append((outer_diameter, shell_diameter, i, length, total_P_drop, delta_P_s, dirt_factor))
            else:
                pass

results.sort(key=lambda x: x[4])
sayac = 1
for outer_diameter, shell_diameter, i, length, total_P_drop, delta_P_s, dirt_factor in results:
    if sayac <= 50:
        print("Outer dia: {} m , Shell dia: {} m , Total Tube Number: {}, Lenght: {} m , delta P total: {} Pa, Delta Ps: {} Pa, Dirt factor: {} ".format(outer_diameter, shell_diameter, i, length, total_P_drop, delta_P_s,dirt_factor))
        sayac += 1
    else:
        pass
print("\n\n\n\n********************-----------------------------------------------------------------------------------------------------------------------********************")
# The choosen one. Result number 18.
outer_diameter = 0.0381
shell_diameter = 0.889
inner_diameter = 0.0381-0.000381
tube_number = 172
length = 6.096

cross_flow_area =area_for_cross_flow_5(outer_diameter,shell_diameter)
print("Sc: ", cross_flow_area)
Gc = Gc_calculator_6(outer_diameter,shell_diameter)
print("Gc: ", Gc)
a_t = flow_area_per_tube_7(inner_diameter)
print("a_t: ", a_t)
battle_window_area = area_for_baffle_window_8(tube_number,shell_diameter,outer_diameter)
print("Sb: ", battle_window_area)
Gb= Gb_calculator_9( tube_number,shell_diameter,outer_diameter)
print("Gb: ", Gb)
flow_area_per_pass = flow_area_per_pass_10(tube_number,inner_diameter)
print("A_t: ",flow_area_per_pass)
Ge = mass_velocity_Ge_calculator_11(outer_diameter,shell_diameter,tube_number)
print("Ge: ", Ge)
average_velocity = average_velocity_calculator_12(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("v : ", average_velocity)
Reynolds_s = Reynolds_s_calculator_13(outer_diameter)
print("Reynolds_s: ", Reynolds_s)
Reynolds_t = Reynolds_t_calculator_14(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("Reynolds_t: ", Reynolds_t)
Pr_s = Pr_s_calculator_15()
print("Pr_s: ", Pr_s)
Pr_t = Pr_t_calculator_16()
print("Pr_t: ", Pr_t)
fi_s = fi_s_calculator_17()
print("fi_s: ", fi_s)
Hs = Hs_calculator_with_donohue_18(outer_diameter,shell_diameter,tube_number)
print("Hs and Ho: ", Hs)
U_d = design_overall_coeff_calculator_21(tube_number,length,outer_diameter)
print("U_d: ", U_d)
A , Re_t , Re_d = Gnielinski_A_calculator_23(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("A: ", A)
print("Re_t: ", Re_t)
print("Re_d: ", Re_d)
f , red = Gnielinski_f_calculator_24(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("Gnielinski f: ", f)
Nu = Gnielinski_Nu_calculator_25(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("Nu: ",Nu)
Hi = Hi_calculator_with_donohue_26(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("Hi: ", Hi)
H_io = H_io_calculator_27(tube_number,inner_diameter,outer_diameter,shell_diameter)
print("Hio: " , H_io)
U_c = clean_overall_coeff_calculator_20(inner_diameter, outer_diameter,shell_diameter,tube_number)
print("U_c: ", U_c)
R_d = dirt_factor_calculator_22(tube_number,length,outer_diameter,inner_diameter,shell_diameter)
print("R_d: ", R_d)
print("***** Pressure part *******")
Re , ab, cd = Reynolds_number_calculator_28(outer_diameter,shell_diameter)
print("Re: ", Re)
f , D_h, zy  = f_calculator_with_Re_29(outer_diameter,shell_diameter)
print("f for pressure ",f)
print("D_h for pressure ",D_h)
N_plus_1 = number_of_crosses_30(shell_diameter,length)
print("N_plus_1: ", N_plus_1)
delta_P_t = inside_tube_t_delta_P_calc_31(length,inner_diameter,tube_number,outer_diameter,shell_diameter)
print("delta_P_t: ", delta_P_t)
delta_P_r = tube_side_r_return_delta_P_calc_32( tube_number, inner_diameter,outer_diameter, shell_diameter)
print("delta_P_r: ", delta_P_r)
print("Total P drop: ", delta_P_t+delta_P_r)
delta_P_s_drop = delta_P_s_drop_calculator_34(outer_diameter,shell_diameter,length)
print("delta_P_s_drop: ", delta_P_s_drop)
